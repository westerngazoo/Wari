...
//!
//! Cherry-picked from `goose-os/kernel/src/page_table.rs` at goose-os
...
//!   - No `unsafe` in this module — `walk()` takes a `read` closure so
//!     the caller decides how to resolve a PA to a u64 (real memory in
//!     the kernel, a HashMap in host tests). INV-9 (slice-to-struct
//!     reinterpretation) does not apply here: there is no `&[u8]`
//!     cast in this file.

// `PAGE_SIZE` is referenced by the `#[cfg(kani)]` proofs and by the
// `#[cfg(test)]` host tests. Keep the import unconditionally — gating
// it on `cfg(any(test, kani))` would be a Surgical-Changes violation
// (the original goose-os file imports it the same way).
#[allow(unused_imports)]
use crate::page_alloc::PAGE_SIZE;

/// Number of entries per page table (`2^9 = 512` for Sv39).
...
///     superpage — rejected for simplicity; Phase 0 kernel only emits
///     4K leaves)
...
pub fn walk<F: FnMut(usize) -> u64>(root: usize, va: usize, mut read: F) -> Option<WalkResult> {
    ...
    for (level_from_top, &idx) in indices.iter().enumerate() {
        ...
        if pte.is_leaf() {
            // Phase 0 only supports 4K leaves (no 1G/2M superpages).
...
mod tests {
    ...
    // PAGE_SIZE is re-checked here so the import is exercised.
    #[test]

...
//!
//! Cherry-picked from `goose-os/kernel/src/page_alloc.rs` at goose-os
//! rev `69d9908b6956315684c567fb95cec542062a61a5` under the "only copy
//! what makes sense" discipline (Wari §Code Quality).
//!
...
//!   - Pure logic: no MMIO, no linker symbols in the allocator core
//!   - Kernel integration (linker-symbol-driven `init()`) lives in
//!     `kernel/src/mem/kvm.rs` (Phase 0a PR 3), not here.

// Cherry-pick-fidelity allows: `% PAGE_SIZE != 0` and `(n + 63) / 64`
// match the goose-os source verbatim and the formal-verification path
// reasons about them as plain integer arithmetic. Switching to
// `is_multiple_of` / `div_ceil` would be a Surgical-Changes violation
// (touching code outside the cherry-pick's purpose). Revisit when a
// downstream change touches the same lines for an unrelated reason.
#![allow(clippy::manual_is_multiple_of)]
#![allow(clippy::manual_div_ceil)]

/// Size of one physical page, in bytes (RISC-V 4 KiB).
...
pub enum AllocError {
    /// All managed pages are allocated.
    OutOfMemory,
    /// `free()` was called on a page that is not currently allocated.
    DoubleFree,
    /// Address is not within `[base, base + total_pages * PAGE_SIZE)`.
    InvalidAddress,
...
// Lives in .bss as zeroed memory, initialized once at boot by the
// kernel's `kvm` module (which reads linker symbols and constructs the
// allocator). After init, accessible from anywhere via `get()`.
...
/// see INV-1) and that `init()` has already constructed `ALLOC` (INV-8).
pub unsafe fn get() -> &'static mut BitmapAllocator {
    // SAFETY: INV-1, INV-8. Caller responsibility documented above.
    unsafe { &mut *core::ptr::addr_of_mut!(ALLOC) }
}

/// Install a fully-constructed allocator into the global slot.
///
/// Called once during boot from `kernel/src/mem/kvm.rs::init()` after
/// reading linker symbols. Not for use after boot.
///
/// # Safety
/// Caller must ensure this is called exactly once, before any call to
/// `get()`, with interrupts disabled (INV-1, INV-8).
pub unsafe fn install(allocator: BitmapAllocator) {
    // SAFETY: INV-1, INV-8. Caller responsibility documented above.
    unsafe {
        core::ptr::addr_of_mut!(ALLOC).write(allocator);
    }
}
...
impl BitmapAllocator {
    ...
    ///
    /// # Panics
    /// `init()` (called separately) panics if `num_pages > MAX_PAGES` or
    ...
    ///
    /// # Safety
    /// Caller must ensure `addr` is a valid, mapped, writable physical
    /// address. Soundness rests on INV-5 (allocator returns kernel-
    /// writable PAs).
    pub unsafe fn zero_page(addr: usize) {
        // SAFETY: INV-5. Caller guarantees `addr` came from this
        // allocator and identity-mapping is RW for the allocator's range.
        let ptr = addr as *mut u8;
        for i in 0..PAGE_SIZE {
            // SAFETY: INV-5. `addr` is allocator-owned, identity-mapped RW;
            // `i < PAGE_SIZE` so `ptr.add(i)` stays within that page.
...
mod tests {
    ...
    fn make_alloc() -> BitmapAllocator {
        let a = BitmapAllocator::new(TEST_BASE, TEST_PAGES);
        a.init();
        a
    }

...
use crate::pool::{BoundedQueue, Pool};
...
impl Endpoint {
    /// Fresh endpoint: empty queues, zero refcount.
...
impl Notification {
    /// Fresh notification: no pending signals, no waiter, zero refcount.
    pub const fn new() -> Self {
        Self {
            signals: 0,
            waiter: None,
...
impl Frame {
    /// Frame over the 4 KiB page at `pa`, zero refcount.
...
impl Untyped {
    /// Untyped pool at `pa` of `1 << size_bits` bytes, nothing retyped.
...
impl Net {
    /// NIC handle for `nic_kind`, not yet initialized, zero refcount.
...
pub struct Socket {
    ...
    /// Local IPv4 address (big-endian; 0 = unbound).
    pub local_ip: u32,
    /// Local port (0 = unbound).
    pub local_port: u16,
    /// Peer IPv4 address (big-endian; 0 = not connected).
    pub peer_ip: u32,
    /// Peer port (0 = not connected).
    pub peer_port: u16,
    /// Cap refcount.
    pub refcount: u16,
}
...
impl Socket {
    /// Unbound socket on the NIC at `net_idx`, wrapping `smoltcp_handle`.
...
pub struct ObjectPools {
    ...
    /// Mappable 4 KiB physical pages.
    pub frames: Pool<Frame, FRAME_POOL_CAPACITY>,
    /// Retypable untyped-memory pools.
    pub untypeds: Pool<Untyped, UNTYPED_POOL_CAPACITY>,
    /// NIC handles (driver-only, INV-19).
    pub nets: Pool<Net, NET_POOL_CAPACITY>,
    /// Per-tenant sockets minted by the net driver.
    pub sockets: Pool<Socket, SOCKET_POOL_CAPACITY>,
}

impl ObjectPools {
    /// Fresh pools, every slot empty. `const fn` for static init.
    pub const fn new() -> Self {
        Self {
            endpoints: Pool::new(),

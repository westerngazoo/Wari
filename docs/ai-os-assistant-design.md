...
>
> **Build status (2026-07):** the substrate this design assumes now
> exists and runs on silicon. The Planner→Executor channel (§4.5) is a
> synchronous `ipc_call` on an Endpoint — proven cross-tenant on the VF2
> (the PING→PONG demo). The Executor's mediation policy (§5) ships as the
> `wari-policy` crate: a pure `evaluate(req, budget) -> Decision`
> gauntlet, merged. What remains is wiring the two roles as modules with
> the right cap grants — the first agentic brick, next on the kernel
> lane.
